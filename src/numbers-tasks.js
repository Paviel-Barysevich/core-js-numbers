...
function getAngleBetweenVectors(x1, y1, x2, y2) {
  const a = (x1 ** 2 + y1 ** 2) ** 0.5;
  const b = (x2 ** 2 + y2 ** 2) ** 0.5;
...
function getSumOfDigits(num) {
  return `${num}`.split('').reduce((sum, d) => sum + +d, 0);
}
...
function getSumOfNumbers(...args) {
  ...
  return arr.reduce((sum, value) => sum + value * 1000, 0) / 1000;
}
...
function getRandomInteger(min, max) {
  ...
  const randomNumber = Math.trunc(((max - min) * random) / random + min);
  return randomNumber;
}
